...
module.exports = {
  ...
  async checkAllNotDeletedAgents(countBefore) {
    ...
    /* we are using count 6 because we have two agents for RDS Instance also,
    hence (pmm-agent, Node exporter, postgres exporter, QAN RDS, QAN postgres, RDS exporter)
     */
    assert.ok((otherDetails <= 6 && otherDetails >= 4), 'Total Agents running on PMM-Server Instance can not be greater then 6');
