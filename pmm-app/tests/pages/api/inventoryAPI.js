...
module.exports = {
    ...
    async verifyServiceExistsAndHasRunningStatus(service, serviceName) {
        ...
        for (let i = 0; i < 30; i++) {
            ...
            if (typeof responseService !== 'undefined') break;
            await new Promise(r => setTimeout(r, 1000));
        }
        assert.ok(typeof responseService !== 'undefined', `Service ${serviceName} was not found`);
    ...
    async waitForRunningState(serviceId) {
        ...
        for (let i = 0; i < 30; i++) {
            ...
            let areRunning = Object.values(agents.data).flat(Infinity).every(({status}) => status === 'RUNNING');
            if (areRunning) {
                return agents
            }
            await new Promise(r => setTimeout(r, 1000));
