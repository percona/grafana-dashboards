...
import { getExample, getExampleItem } from './Example.tools';
...
const Example: FC<ExampleInterface> = ({ databaseType, examples, loading }) => {
  const isExample = examples && examples.filter((example) => getExampleItem(databaseType, example)).length;
    ...
    <Overlay isPending={loading} size={OVERLAY_LOADER_SIZE}>
      {isExample && !loading
        ? examples
          ...
          .map((example) => getExampleItem(databaseType, example))
