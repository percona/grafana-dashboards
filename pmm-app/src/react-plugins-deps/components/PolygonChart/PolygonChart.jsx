import React, { BaseSyntheticEvent as currentEvent, Component } from "react";
...
class PolygonChart extends Component {
  ...
  drawPolygonChart() {
    ...
    const scaleX = scaleLinear()
      .domain([
        this.findMinX(this.appLoadPolygonChart),
        this.findHighestX(this.appLoadPolygonChart)
      ])
      .range([0, xAxisLength]);

    const scaleY = scaleLinear()
      .domain([
        this.findHighestY(this.appLoadPolygonChart),
        this.findMinY(this.appLoadPolygonChart)
      ])
      .range([0, yAxisLength]);
    ...
    const bisectDate = bisector((d, x) => +moment.utc(d[this.xkey]).isBefore(x))
      .right;
  ...
  render() {
    return (
      <div className={"d3-bar-chart-container"} ref={"graphContainer"}></div>
    );
  }
}
