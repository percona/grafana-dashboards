...
build:
	cd pmm-mongo-app && npm run build && cd ..
	cd pmm-mysql-app && npm run build && cd ..
	cd pmm-qan-app && npm run build && cd ..
	cd pmm-summary-app && npm run build && cd ..
	cd pmm-overview-app && npm run build && cd ..
	tar czf pmm-apps.tar.gz pmm-mongo-app pmm-mysql-app pmm-qan-app pmm-summary-app pmm-overview-app
...
disable:
	curl -X POST 'http://admin:admin@localhost/graph/api/plugins/pmm-overview-app/settings' -d 'enabled=false'
	curl -X POST 'http://admin:admin@localhost/graph/api/plugins/pmm-mongo-app/settings' -d 'enabled=false'
	curl -X POST 'http://admin:admin@localhost/graph/api/plugins/pmm-system-summary-app/settings' -d 'enabled=false'
	curl -X POST 'http://admin:admin@localhost/graph/api/plugins/pmm-mysql-app/settings' -d 'enabled=false'
	curl -X POST 'http://admin:admin@localhost/graph/api/plugins/pmm-qan-app/settings' -d 'enabled=false'

enable:
	curl -X POST 'http://admin:admin@localhost/graph/api/plugins/pmm-overview-app/settings' -d 'enabled=true'
	curl -X POST 'http://admin:admin@localhost/graph/api/plugins/pmm-mongo-app/settings' -d 'enabled=true'
	...
	curl -X POST 'http://admin:admin@localhost/graph/api/plugins/pmm-qan-app/settings' -d 'enabled=true'
