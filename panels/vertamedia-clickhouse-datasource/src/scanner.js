...
var Scanner = /** @class */ (function () {
    ...
    Scanner.prototype.parseJOIN = function (argument) {
        ...
        if (isClosureChars(this.token)) {
            var subQuery = betweenBraces(this._s);
        ...
        var joinAST = { type: joinType, source: source, aliases: [], using: [], on: [] };
        do {
        ...
        var joinExprToken = toLower(this.token);
        var joinConditions = "";
        while (this.next()) {
    ...
    Scanner.prototype.removeComments = function (query) {
        return query.replace(new RegExp(commentRe, 'g'), '');
    };
    return Scanner;
}());
export default Scanner;
var wsRe = "\\s+", commentRe = "--[^\n]*|/\\*(?:[^*]|\\*[^/])*\\*/", idRe = "[a-zA-Z_][a-zA-Z_0-9]*", intRe = "\\d+", powerIntRe = "\\d+e\\d+", floatRe = "\\d+\\.\\d*|\\d*\\.\\d+|\\d+[eE][-+]\\d+", stringRe = "('(?:[^'\\\\]|\\\\.)*')|(`(?:[^`\\\\]|\\\\.)*`)|(\"(?:[^\"\\\\]|\\\\.)*\")", binaryOpRe = "=>|\\|\\||>=|<=|==|!=|<>|->|[-+/%*=<>\\.!]", statementRe = "\\b(with|select|from|where|having|order by|group by|limit|format|prewhere|union all)\\b", 
...
joinsRe = "\\b(" +
    ...
    "outer\\s+join|" +
    "join" +
    ")\\b", onJoinTokenRe = '\\b(using|on)\\b', tableNameRe = '([A-Za-z0-9_]+|[A-Za-z0-9_]+\\.[A-Za-z0-9_]+)', macroFuncRe = "(\\$rateColumns|\\$perSecondColumns|\\$rate|\\$perSecond|\\$columns)", condRe = "\\b(or|and)\\b", inRe = "\\b(global in|global not in|not in|in)\\b", closureRe = "[\\(\\)\\[\\]]", specCharsRe = "[,?:]", macroRe = "\\$[A-Za-z0-9_$]+", skipSpaceRe = "[\\(\\.! \\[]", tableFuncRe = "\\b(remote|remoteSecure|cluster|clusterAllReplicas|merge|numbers|url|mysql|jdbc|odbc|hdfs|input|generateRandom)\\b", builtInFuncRe = "\\b(avg|countIf|first|last|max|min|sum|sumIf|ucase|lcase|mid|round|rank|now|" +
